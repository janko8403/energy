...
if(isset($_GET['idClient'])) {
    ...
    $sql = "SELECT idOneNet_measurements, idDevice, idClient, last_power_value, timestamp, requested_timestamp
        ...
        $sql9 = "SELECT * FROM (SELECT idOneNet_measurements, idDevice, 99 as idClient, last_power_value, timestamp, sum(last_power_value) as lpv, requested_timestamp
        FROM OneNet.OneNet_measurements
                WHERE #idClient = 8 and
                requested_timestamp > now() - interval 1 hour
                ...
                while($row = $query9->fetch() ) :
                    ...
                    $timestamp = date('Y-m-d H:i:00',$requested_timestamp);
                                        ...
                                        echo '<br>';echo $timestamp; echo ' ' ;echo ' <br>';
                    echo '<br>';echo gmdate("Y-m-d H:i:s", $requested_timestamp); echo ' ' ; echo $y++; echo ' <br>';
                    $tmpArray9['id']                 = $id;
                    $tmpArray9['idDevice']           = "https://enspirion.pl";
                    $tmpArray9['idClient']           = 99;
                    ...
                    echo '<br>';echo gmdate("Y-m-d H:i:s", $requested_timestamp); echo ' ' ; echo $x++; echo ' <br>';
...
echo '<br>';
        ...
        echo '<br>';
        echo '<br>';
        echo $now - date_create()->format('Y-m-d H:i:s');
