...
use App\Models\{MeasurementOneMinute, Configuration};
use Illuminate\Http\Request;
use Carbon\Carbon;
use Illuminate\Support\Facades\DB;


class GetOneMinute implements ActionInterface
{
    ...
    public function execute($data)
    {
        ...
        $sums = MeasurementOneMinute::select(
        ...
        $arrS = [];
        foreach($sums as $s) {
            $vals = MeasurementOneMinute::select(
    ...
    public function heading($data)
    {
        ...
        $arrClient = [];
        foreach($data->clients as $client) {
            $getUser = MeasurementOneMinute::select(
                "client_id"
            )
            ->whereBetween('requested_timestamp', [$data->from, $to])
