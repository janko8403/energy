...
use App\Models\{MeasurementFifteenMinute, Configuration};
use Illuminate\Http\Request;
use Carbon\Carbon;
use Illuminate\Support\Facades\DB;


class GetFifteenMinute implements ActionInterface
{
    ...
    public function execute($data)
    {
        ...
        $sums = MeasurementFifteenMinute::select(
        ...
        $arrS = [];
        foreach($sums as $s) {
            $vals = MeasurementFifteenMinute::select(
    ...
    public function heading($data)
    {
        ...
        $arrClient = [];
        foreach($data->clients as $client) {
            $getUser = MeasurementFifteenMinute::select(
                "client_id"
            )
            ->whereBetween('requested_timestamp', [$data->from, $to])
